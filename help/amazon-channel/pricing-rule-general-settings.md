...
1. Wählen Sie für **[!UICONTROL Status]** eine Option:
   ...
   - `Active` - Wählen Sie diese Option aus, wenn die Preisregel auf Ihre förderfähigen Produkte angewendet werden soll, und passen Sie Ihre Listenpreise vor der Veröffentlichung in Amazon an.

   - `Inactive` - Wählen Sie diese Option, wenn die Preisregel nicht auf Ihre förderfähigen Produkte angewendet werden soll. Diese Option wird höchstwahrscheinlich verwendet, wenn eine Preisregel geändert oder nach einer begrenzten Promotion deaktiviert wird.
...
>[!NOTE]
>
>Um nachfolgende Regeln zu verwerfen, muss eine Preisregel einen definierten Wert für **Priorität** haben.

![Allgemeine Einstellungen der Preisregel](assets/amazon-pricing-rule-general.png){width="600" zoomable="yes"}
...
| [!UICONTROL From] | Geben Sie das Startdatum an, an dem die Preisregel beginnt. Wenn Sie beispielsweise einen Verkauf im letzten Monat des Jahres durchführen möchten, würden Sie das `From`-Datum auf den 1. Dezember setzen, sodass die Preisregel ab dem 1. Dezember automatisch auf Ihre Amazon-Listen angewendet wird. |
